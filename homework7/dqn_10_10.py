...
class Agent:
    ...
    def replay(self):
        ...
        self.model.fit(X[1:,],Y[1:,], batch_size=self.batch_size, epochs=10, verbose=False)
...
if __name__ == "__main__":
    ...
    for step in range(max_iter):
        ...
        # epsilon decay
        if step % smart_mouse.update_model_freq == 0 and step>0:
            print("average reward in one step", total_reward/smart_mouse.update_model_freq)
            total_reward = 0
            smart_mouse.replay()
        if step % smart_mouse.update_target_freq == 0 and step>0:
