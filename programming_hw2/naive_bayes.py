...
class BernoulliNaiveBayes:
    ...
    def train(self, x_train, y_train):
        ...
        num_y1 = sum(y_train)
        num_y0 = y_train.shape[0] - num_y1
        self.p_y1 = num_y1 / y_train.shape[0]
        self.p_y0 = 1 - self.p_y1

        # learn the p(x|y=0), p(x|y=1), they should be a vector with shape (# of feature, ).
        # Laplace smoothing
        y1_set = x_train[np.where(y_train > 0)]
        y0_set = x_train[np.where(y_train < 1)]
    ...
    def predict(self, x_test):
        ...
        p0 = np.sum(np.log(self.p_x_y0 * x_test + (1 - self.p_x_y0) * (1 - x_test)), axis=1)
        p1 = np.sum(np.log(self.p_x_y1 * x_test + (1 - self.p_x_y1) * (1 - x_test)), axis=1)
